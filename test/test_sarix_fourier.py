...
def test_prediction_statistical_similarity():
    """Test that predictions from two runs are statistically similar

    This test acknowledges that JAX's RNG may not produce identical results,
    but checks that predictions are 'roughly similar' by comparing:
    - Mean predictions within reasonable tolerance
    - Standard deviations are similar
    - High correlation between predictions
    """
    xy = np.random.randn(3, 25, 2) + 10
    ...
    # Compare mean predictions across samples
    mean1 = np.mean(model1.predictions, axis=0)  # Average over samples
    mean2 = np.mean(model2.predictions, axis=0)

    # Mean predictions should be close (within 20% relative error on average)
    relative_error = np.abs(mean1 - mean2) / (np.abs(mean1) + 1e-8)
    assert np.mean(relative_error) < 0.20, f"Mean relative error too large: {np.mean(relative_error)}"

    # Standard deviations should be similar
    std1 = np.std(model1.predictions, axis=0)
    std2 = np.std(model2.predictions, axis=0)
    std_relative_error = np.abs(std1 - std2) / (np.abs(std1) + 1e-8)
    assert np.mean(std_relative_error) < 0.30, f"Std relative error too large: {np.mean(std_relative_error)}"

    # Predictions should be highly correlated
    # Flatten predictions for correlation calculation
    pred1_flat = model1.predictions.flatten()
    pred2_flat = model2.predictions.flatten()
    correlation = np.corrcoef(pred1_flat, pred2_flat)[0, 1]
    assert correlation > 0.85, f"Correlation too low: {correlation}"


def test_parameter_consistency():
    """Test that estimated parameters are similar between runs

    Checks that theta and sigma samples have similar distributions
    """
    xy = np.random.randn(3, 25, 2) + 10
    ...
    # Compare theta parameter means
    theta1_mean = np.mean(model1.samples['theta'], axis=0)
    theta2_mean = np.mean(model2.samples['theta'], axis=0)
    theta_rel_error = np.abs(theta1_mean - theta2_mean) / (np.abs(theta1_mean) + 1e-8)
    assert np.mean(theta_rel_error) < 0.25, f"Theta means differ too much: {np.mean(theta_rel_error)}"

    # Compare sigma parameter means
    sigma1_mean = np.mean(model1.samples['sigma'], axis=0)
    sigma2_mean = np.mean(model2.samples['sigma'], axis=0)
    sigma_rel_error = np.abs(sigma1_mean - sigma2_mean) / (np.abs(sigma1_mean) + 1e-8)
    assert np.mean(sigma_rel_error) < 0.25, f"Sigma means differ too much: {np.mean(sigma_rel_error)}"

    # Check theta samples are highly correlated
    theta1_flat = model1.samples['theta'].flatten()
    theta2_flat = model2.samples['theta'].flatten()
    theta_corr = np.corrcoef(theta1_flat, theta2_flat)[0, 1]
    assert theta_corr > 0.80, f"Theta correlation too low: {theta_corr}"
